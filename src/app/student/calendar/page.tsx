...
export default function CalendarPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white mb-2">カレンダー</h1>
        <p className="text-zinc-400">コーチングセッションのスケジュール</p>
      </div>

      {/* Upcoming Events */}
      <div className="mb-8">
        <h2 className="text-lg font-semibold text-white mb-4">予定のセッション</h2>
        {upcomingEvents.length > 0 ? (
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="text-zinc-300 mb-1">{formatDateTime(event.startTime)}</p>
                    <p className="text-sm text-zinc-500">コーチ: {event.coach?.name}</p>
                  </div>
                  {event.status === 'scheduled' && (
                    <button
                      ...
                      className="text-sm text-zinc-400 hover:text-white transition-colors px-4 py-2 bg-zinc-800 rounded-lg hover:bg-zinc-700 whitespace-nowrap"
                    >
                      変更リクエスト
                    </button>
                  )}
          ...
          <div className="bg-zinc-900 rounded-xl p-12 border border-zinc-800 text-center">
            ...
            <p className="text-zinc-400">予定されたセッションはありません</p>
          </div>
        )}
      </div>
      ...
      {pastEvents.length > 0 && (
        <div>
          <h2 className="text-lg font-semibold text-white mb-4">過去のセッション</h2>
          <div className="space-y-2">
            {pastEvents.slice(0, 10).map((event) => (
              <div
                key={event.id}
                className="bg-zinc-900/50 rounded-lg p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2"
              >
                <div>
                  <p className="font-medium text-zinc-300">{event.title}</p>
                  <p className="text-sm text-zinc-500">{formatDateTime(event.startTime)}</p>
                </div>
      ...
      {showChangeModal && selectedEvent && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-zinc-900 rounded-xl p-6 w-full max-w-md border border-zinc-800">
            <h3 className="text-lg font-semibold text-white mb-4">日程変更リクエスト</h3>
            <div className="space-y-4">
              <div>
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                className="flex-1 px-4 py-3 text-zinc-400 hover:text-white bg-zinc-800 hover:bg-zinc-700 rounded-lg transition-colors"
              >
                キャンセル
              </button>
              <button
