  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <Link href="/student/vocab" className="text-gray-500 hover:text-gray-800 text-sm mb-2 inline-flex items-center gap-1">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
            単語帳一覧
          </Link>
          <h1 className="text-2xl font-bold text-gray-800">{data.deck.title}</h1>
        </div>
        {mode === 'list' && (
          <div className="flex gap-3">
            ...
            <Link
              ...
              className="px-4 py-2.5 bg-gray-100 text-gray-800 rounded-lg font-medium hover:bg-gray-200 transition-colors border border-gray-300"
            >
              テストを受ける
            </Link>
          </div>
        ...
        <div className="max-w-xl mx-auto">
          <div className="bg-white rounded-xl p-8 border border-gray-200 text-center">
            ...
            {currentCard.pronunciation && (
              <p className="text-gray-500 mb-4">{currentCard.pronunciation}</p>
            )}

            {showAnswer ? (
              <div className="mt-8">
                <p className="text-xl text-gray-600 mb-3">{currentCard.meaning}</p>
                {currentCard.example && (
                  <p className="text-sm text-gray-500 italic">例: {currentCard.example}</p>
                )}
              ...
              <button
                ...
                className="mt-8 px-8 py-3 bg-gray-100 text-gray-800 rounded-lg hover:bg-gray-200 transition-colors font-medium"
              >
                答えを見る
              </button>
            )}
          ...
          <button
            ...
            className="mt-4 text-gray-500 hover:text-gray-800 text-sm transition-colors"
          >
            学習を終了
          </button>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            {data.cards.map((card) => (
              <div key={card.index} className="p-4">
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div>
                    <span className="font-medium text-gray-800">{card.word}</span>
                    {card.pronunciation && (
                      <span className="text-gray-500 text-sm ml-2">{card.pronunciation}</span>
                    )}
                  </div>
                ...
                <p className="text-gray-500 text-sm">{card.meaning}</p>
              </div>
            ))}
          </div>
          ...
          <table className="w-full hidden lg:table">
            <thead className="bg-gray-100/50">
              <tr>
                <th className="px-6 py-3 text-left text-sm font-medium text-gray-500">単語</th>
                <th className="px-6 py-3 text-left text-sm font-medium text-gray-500">意味</th>
                <th className="px-6 py-3 text-left text-sm font-medium text-gray-500">状態</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-800">
              {data.cards.map((card) => (
                <tr key={card.index} className="hover:bg-gray-100/30 transition-colors">
                  ...
                  <td className="px-6 py-4 text-gray-500">{card.meaning}</td>
                  <td className="px-6 py-4">
